...
public class FormatUtils {
    ...
    public static void appendPaddedInteger(Appendable appendable, int i, int i2) {
        ...
        if (i < 10) {
            ...
            int i4 = i < 1000 ? 3 : i < 10000 ? 4 : ((int) (Math.log((double) i) / a)) + 1;
            while (i2 > i4) {
                appendable.append(TarjetasConstants.ULT_NUM_AMEX);
                i2--;
            }
    ...
    public static void appendPaddedInteger(Appendable appendable, long j, int i) {
        ...
        if (((long) i2) == j) {
            ...
            int log = ((int) (Math.log((double) j) / a)) + 1;
            while (i > log) {
                appendable.append(TarjetasConstants.ULT_NUM_AMEX);
                i--;
            }
    ...
    public static int calculateDigitCount(long j) {
        ...
        if (j >= 0) {
            if (j >= 10) {
                i = j < 100 ? 2 : j < 1000 ? 3 : j < LocalizacionManager.UPDATE_INTERVAL_IN_MILLISECONDS ? 4 : 1 + ((int) (Math.log((double) j) / a));
            }
            return i;
