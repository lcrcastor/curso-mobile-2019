...
public class PKCS12Util {
    ...
    public static byte[] convertToDefiniteLength(byte[] bArr, char[] cArr, String str) {
        ...
        try {
            ...
            Pfx pfx = new Pfx(contentInfo, new MacData(new DigestInfo(new AlgorithmIdentifier(macData.getMac().getAlgorithmId().getObjectId(), DERNull.INSTANCE), a(macData.getMac().getAlgorithmId().getObjectId(), macData.getSalt(), intValue, cArr, ASN1OctetString.getInstance(contentInfo.getContent()).getOctets(), str)), macData.getSalt(), intValue));
            byteArrayOutputStream.reset();
