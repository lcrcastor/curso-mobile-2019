...
public final class ImmutableValueGraph<N, V> extends ValueBackedImpl<N, V> implements ValueGraph<N, V> {
    private ImmutableValueGraph(ValueGraph<N, V> valueGraph) {
        super(ValueGraphBuilder.from(valueGraph), a(valueGraph), (long) valueGraph.edges().size());
    }
    ...
    private static <N, V> ImmutableMap<N, GraphConnections<N, V>> a(ValueGraph<N, V> valueGraph) {
        ...
        for (Object next : valueGraph.nodes()) {
            builder.put(next, a(valueGraph, next));
        }
    ...
    private static <N, V> GraphConnections<N, V> a(final ValueGraph<N, V> valueGraph, final N n) {
        ...
        return valueGraph.isDirected() ? DirectedGraphConnections.a(valueGraph.predecessors(n), Maps.asMap(valueGraph.successors(n), (Function<? super K, V>) r0)) : UndirectedGraphConnections.a(Maps.asMap(valueGraph.adjacentNodes(n), (Function<? super K, V>) r0));
    }
