...
public abstract class ImmutableGraph<N> extends ForwardingGraph<N> {
    ...
    public static <N> ImmutableGraph<N> copyOf(Graph<N> graph) {
        return graph instanceof ImmutableGraph ? (ImmutableGraph) graph : new ValueBackedImpl(GraphBuilder.from(graph), a(graph), (long) graph.edges().size());
    }
    ...
    private static <N> ImmutableMap<N, GraphConnections<N, Presence>> a(Graph<N> graph) {
        Builder builder = ImmutableMap.builder();
        for (Object next : graph.nodes()) {
            builder.put(next, a(graph, next));
        }
        return builder.build();
    }

    private static <N> GraphConnections<N, Presence> a(Graph<N> graph, N n) {
